<template>
  <div class="login">
    ...
    <form action="">
        ...
        <input type="submit" class="submit" @keyup.enter="login"  @click="login" value="登录" />
    </form>
          ...
          password_sha(pwd){
            ...
            login:function(){
               if (this.username && this.pwd) {
                ...
                loginParam.append('password', password)
                  //请求后端
                this.$http( 
                //'http://localhost:8088/api/login',//这种配置方式不清晰
                 //{param: loginParam}////此种方式会发送两次请求并且数据在里request playload里而不在formdata里
                 {
                    method:'post',
                    headers: {
								        'x-access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InlteCIsInBhc3N3b3JkIjoiYTY2NWE0NTkyMDQyMmY5ZDQxN2U0ODY3ZWZkYzRmYjhhMDRhMWYzZmZmMWZhMDdlOTk4ZTg2ZjdmN2EyN2FlMyIsImNyZWF0ZWRfYXQiOm51bGwsInVwZGF0ZWRfYXQiOm51bGwsImlkIjozLCJpYXQiOjE1MzY1NDk3OTYsImV4cCI6MTUzNjYzNjE5Nn0.xuOMs9IO5wEnbfgJ-COi2O1G9ii3UPDfsoLXb1K8Hug',
								    },
...
//                  	首次登陆成功获取token并存储
												if(response.data.token != null){
													console.log('赋值前'+response.data.token)
													this.token = response.data.token
													console.log('赋值后'+response.data.token)
